...
import (
	...
	cinder "github.com/openstack-k8s-operators/cinder-operator/api/v1beta1"
...
var _ = BeforeSuite(func() {
	...
	err = cinder.AddToScheme(scheme.Scheme)
	Expect(err).NotTo(HaveOccurred())
	...
	cinder.SetupDefaults()

	kclient, err := kubernetes.NewForConfig(cfg)
	Expect(err).ToNot(HaveOccurred(), "failed to create kclient")
	err = (&controllers.CinderReconciler{
	...
	err = (&controllers.CinderSchedulerReconciler{
		Client:  k8sManager.GetClient(),
		Scheme:  k8sManager.GetScheme(),
		Kclient: kclient,
	}).SetupWithManager(context.Background(), k8sManager)
	...
	err = (&controllers.CinderVolumeReconciler{
		Client:  k8sManager.GetClient(),
		Scheme:  k8sManager.GetScheme(),
		Kclient: kclient,
	}).SetupWithManager(context.Background(), k8sManager)
	...
	err = (&controllers.CinderBackupReconciler{
		Client:  k8sManager.GetClient(),
		Scheme:  k8sManager.GetScheme(),
		Kclient: kclient,
	}).SetupWithManager(context.Background(), k8sManager)
...
var _ = BeforeEach(func() {
	...
	DeferCleanup(k8sClient.Delete, ctx, CreateCinderSecret(cinderName.Namespace, SecretName))
})
