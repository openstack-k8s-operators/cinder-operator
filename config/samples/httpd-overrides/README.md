# Cinder HTTPD Configuration Overrides

The cinder-operator provides mechanisms to customize the Apache HTTPD server
...
functionality to mount custom HTTPD configuration files into the Cinder
deployment.
...
### 1. Find the Cinder Pod

First, identify the running Cinder pod:

```bash
$ oc get pods -l component=cinder-api
```

### 2. Verify Configuration Loading

Connect to the Cinder Pod and check that your custom configuration has been
loaded:
...
The cinder-operator repository includes a sample that can be used to deploy
cinder with httpd overrides (it set a particular Timeout to 300s). This sample
