...
- admissionReviewVersions:
  ...
  clientConfig:
    caBundle: ${CA_BUNDLE}
    url: https://${CRC_IP}:${WEBHOOK_PORT}/validate-cinder-openstack-org-v1beta1-cinder
  failurePolicy: Fail
...
- admissionReviewVersions:
  ...
  clientConfig:
    caBundle: ${CA_BUNDLE}
    url: https://${CRC_IP}:${WEBHOOK_PORT}/mutate-cinder-openstack-org-v1beta1-cinder
  failurePolicy: Fail
  matchPolicy: Equivalent
  name: mcinder.kb.io
  objectSelector: {}
...
CSV_NAME="$(oc get csv -n openstack-operators -l operators.coreos.com/cinder-operator.openstack-operators -o name)"
...
if [ -n "${CSV_NAME}" ]; then
    ...
    if CR_NAME=$(oc get openstack -n openstack-operators -o name); then
        ...
        To restore it, use:
        oc patch ${CR_NAME} -n openstack-operators --type=merge -p '{\"spec\": {\"operatorOverrides\": [{\"name\": \"cinder\", \"replicas\": 1}]}}'\n"

        oc patch ${CR_NAME} -n openstack-operators --type=merge -p '{"spec": {"operatorOverrides": [{"name": "cinder", "replicas": 0}]}}'
    fi
fi
