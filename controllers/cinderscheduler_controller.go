...
func (r *CinderSchedulerReconciler) reconcileNormal(ctx context.Context, instance *cinderv1beta1.CinderScheduler, helper *helper.Helper) (ctrl.Result, error) {
	...
	//
	// check for required Cinder config maps that should have been created by owner Cinder CR
	//
	...
	if err != nil {
		if k8s_errors.IsNotFound(err) {
			return ctrl.Result{RequeueAfter: time.Second * 10}, fmt.Errorf("Could not find all Cinder config maps for %s", parentCinderName)
		}
		return ctrl.Result{}, err
	}

	// run check CinderScheduler owner Cinder CR config maps - end
