...
// CinderBackupReconciler reconciles a CinderBackup object
...
// Reconcile - cinder backup
func (r *CinderBackupReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	_ = context.Background()
	_ = r.Log.WithValues("cinderbackup", req.NamespacedName)
...
func (r *CinderBackupReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	configMapFn := handler.ToRequestsFunc(func(cm handler.MapObject) []reconcile.Request {
		...
		if err := r.Client.List(context.Background(), backups, listOpts...); err != nil {
			log.Error(err, "Unable to retrieve backup CRs %v")
			return nil
		}
...
func (r *CinderBackupReconciler) statefulsetCreateOrUpdate(instance *cinderv1beta1.CinderBackup, envVars map[string]util.EnvSetter) (controllerutil.OperationResult, error) {
	// set KOLLA_CONFIG env vars
	envVars["KOLLA_CONFIG_FILE"] = util.EnvValue(cinderbackup.KollaConfig)
	...
	op, err := controllerutil.CreateOrUpdate(context.TODO(), r.Client, statefulset, func() error {
		...
		if statefulset.ObjectMeta.CreationTimestamp.IsZero() {
			statefulset.Spec.Selector = &metav1.LabelSelector{
				MatchLabels: common.GetLabels(instance.Name, cinderbackup.AppLabel),
			}
		}
		...
		for k, v := range common.GetLabels(instance.Name, cinderbackup.AppLabel) {
			...
			Containers: []corev1.Container{
				{
					...
					// TODO: probably depends on the used backup backend
					//       for nfs privileges are req to mount the share
					//       fpr now just run privileged.
